...
for(i=0;i<n;i++)
{
wt[i]=tat[i]-bt[i]; // waiting time calculated from the
turnaround time
//burst time
swt=swt+wt[i];
// summation of wait time
stat=stat+tat[i];
...
for(i = 1; i <= n; i++)
{
process[i].turnaround = process[i].wait +
process[i].burst; // calc of
process[i].ratio = (float)process[i].turnaround /
(float)process[i].burst;
printf("%5d %8d %7d %8d %10d %9d %10.1f ", process[i].id,process[i].arrival, process[i].burst, process[i].wait,
process[i].finish,
